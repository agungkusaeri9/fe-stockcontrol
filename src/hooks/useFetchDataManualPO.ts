...
export const useFetchDataManualPurchaseOrder = <T>(
    ...
    const { data, isLoading, refetch } = useQuery<T[]>({
        queryKey: usePagination
            ? [queryKey, currentPage, limit, debouncedSearch,filter.pr_number, filter.po_number, filter.start_date, filter.end_date, filter.kanban]
            : [queryKey],
        queryFn: fetchData,
    });
